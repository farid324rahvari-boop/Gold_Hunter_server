...
async function fetchTFHistory(tf, totalBars) {
  ...
  if (!key || !TF[tf]) {
    historyDiagnostics.set(tf, {
      ...
      stoppedReason: !key ? 'missing-api-key' : 'unsupported-timeframe',
      oldest: null,
      newest: null,
      pageDiagnostics: []
    });
  ...
  for (let page = 0; page < 80 && all.size < wanted; page++) {
    ...
    const result = await fetchJson(cacheKey, url, ttl, 15000);
    const data = result.data;

    if (!data || (typeof data === 'object' && data.status === 'error') || !Array.isArray(data?.values) || !data.values.length) {
      const httpStatus = result.httpStatus ?? result.status ?? null;
      const providerError = result.providerError || (data && typeof data === 'object' ? (data.message || data.code) : null);
      const error = result.error || null;

      if (httpStatus && httpStatus >= 400) stoppedReason = 'provider-http-error';
      else if (providerError) stoppedReason = 'provider-error';
      else if (error) stoppedReason = 'network-error';
    ...
    if (!bars.length) {
      ...
      pages.push({ page: page + 1, received: 0, uniqueAdded: 0, oldest: null, newest: null, error: 'no-valid-bars', boundary });
      break;
    }
    ...
    if (seenBoundaries.has(pageKey)) {
      ...
      pages.push({ page: page + 1, received: bars.length, uniqueAdded: 0, oldest: new Date(oldest).toISOString(), newest: new Date(newest).toISOString(), error: 'repeated-page', boundary });
      break;
    }
    ...
    // Move one full candle before the oldest returned candle. This avoids
    // provider-specific inclusive/exclusive end_date ambiguity.
  ...
  const meta = {
    ...
    provider: {
      httpStatus: lastPage.httpStatus ?? null,
      providerError: lastPage.providerError ?? null,
      error: lastPage.error ?? null,
      cached: !!lastPage.cached,
      stale: !!lastPage.stale
    },

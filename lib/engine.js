...
// A single strategy firing alone is treated as SETUP/WAIT. A live/backtest
// entry needs two independent directional engines to agree. Fibonacci and RSI
// Divergence are context-only and are not counted as trigger confirmations.
const MIN_TRIGGER_CONFIRMATIONS = Math.max(2, Number(process.env.MIN_TRIGGER_CONFIRMATIONS || 2));
...
function trendStrategy(d) {
  ...
  const healthyRsi = dir === 'BUY' ? rrsi >= 48 && rrsi <= 75 : rrsi >= 25 && rrsi <= 48;
  // SELL Trend was materially weaker in the last report. Keep BUY logic intact,
  // but require 1H agreement + cleaner RSI on SELL to reduce counter-trend/late
  // bearish triggers without imposing a 7/7 consensus gate.
  const sellTrendConfirm = dir !== 'SELL' || (h1 === -1 && rrsi <= 48);
  ...
  const setup = pullbackTouched && emaAligned && healthyRsi && notChasing && sellTrendConfirm;
...
function fibonacciStrategy(d, fib) {
  ...
  // Fibonacci is context/setup, not an independent entry trigger.
  // It can confirm another trigger, but should not open a position by itself.
  return { name:'Fibonacci Retracement', strategyId:'FIBONACCI', status:'SETUP', vote:dir, direction:dir, confidence:Math.round(clamp(c)), weight:0, contextVote:dir, reason:fib.reason+'؛ فقط Context', trigger:trig };
}
...
function divergenceStrategy(d, div) {
  ...
  // Divergence is context/setup, not a standalone entry trigger.
  // This preserves its information without letting a weak standalone edge fire trades.
  return {name:'واگرایی RSI',strategyId:'RSI_DIVERGENCE',status:'SETUP',vote:dir,direction:dir,confidence:Math.round(clamp(c)),weight:0,contextVote:dir,reason:div.reason+'؛ فقط Context',trigger:trig};
}
...
function runStrategies(d, fund) {
  ...
  const strategies = [trend, structure, sweep, momentum, fibonacci, divergenceS, fundamental];
  // Only true entry engines participate in trigger confirmation. Fibonacci and
  // RSI Divergence remain context/setup and can never make a 1/7 entry valid.
  const triggerEngines = strategies.filter(s =>
    ['TREND_FOLLOWING','STRUCTURE','LIQUIDITY_SWEEP','MOMENTUM','FUNDAMENTAL'].includes(s.strategyId) &&
    s.status === 'ACTIVE' && ['BUY','SELL'].includes(s.direction)
  );
  const buyActive = triggerEngines.filter(s => s.direction === 'BUY');
  const sellActive = triggerEngines.filter(s => s.direction === 'SELL');
  const direction = buyActive.length > sellActive.length ? 'BUY' : sellActive.length > buyActive.length ? 'SELL' : null;
  const aligned = direction === 'BUY' ? buyActive : direction === 'SELL' ? sellActive : [];
  const triggerPriority = ['TREND_FOLLOWING','STRUCTURE','LIQUIDITY_SWEEP','MOMENTUM','FUNDAMENTAL'];
  const trigger = aligned.length >= MIN_TRIGGER_CONFIRMATIONS
    ? [...aligned].sort((a,b) => triggerPriority.indexOf(a.strategyId) - triggerPriority.indexOf(b.strategyId))[0]
    : null;
  return {
    strategies,
    ...
    triggerConfirmation: {
      required: MIN_TRIGGER_CONFIRMATIONS,
      buy: buyActive.length,
      sell: sellActive.length,
      aligned: aligned.length,
      direction: direction || 'WAIT',
      eligible: triggerEngines.map(s => s.strategyId)
    },
...
function evaluateIndependentQuality(signal, d, result) {
  ...
  else if (signal.strategyId !== 'RSI_DIVERGENCE') { score -= 8; blockers.push('مومنتوم تأیید نمی‌کند'); }
  ...
  score = Math.round(clamp(score));
  // Quality is a real execution gate in both live and historical paths.
  // It remains soft (no 4/7 consensus requirement), but confidence, RR and
  // structural stop geometry are mandatory so weak candidates cannot leak through.
  const confidenceOk = Number(signal.confidence || 0) >= MIN_CONFIDENCE;
  // Quality score is diagnostic/context only. Do not discard an otherwise valid
  // independent trigger merely because the score is below a cosmetic grade.
  // Hard execution safety remains mandatory: confidence, RR and stop geometry.
  const tradable = confidenceOk && rr >= MIN_RR && stopAtr >= MIN_STOP_ATR && stopAtr <= MAX_STOP_ATR;
  if (!confidenceOk) blockers.push(`اعتماد کمتر از ${MIN_CONFIDENCE}`);
...
function getIndependentSignals(d, fund, news = {}) {
  ...
  const active = strategies.filter(s => s.status === 'ACTIVE' && ['BUY','SELL'].includes(s.direction));
  return { strategies, active, volumeConfirmed, context };
}

function decide(d, fund, news, options = {}) {
  const {strategies, volumeConfirmed, context, triggerStrategy, triggerConfirmation}=runStrategies(d,fund);
  const trend = strategies.find(s => s.strategyId === 'TREND_FOLLOWING');
  const activeStrategies = strategies.filter(s => s.status === 'ACTIVE');

  // Independent engines remain independent, but a trade is not allowed on a
  // lone 1/7 trigger. Two independent directional engines must agree; Trend
  // Following keeps priority when it is one of the aligned engines.
  ...
  const blockers=[];
  if (!signal) {
    if (triggerConfirmation.aligned < MIN_TRIGGER_CONFIRMATIONS) {
      blockers.push(`تأیید Trigger کافی نیست: ${triggerConfirmation.aligned}/${triggerConfirmation.required} موتور مستقل هم‌جهت`);
    } else {
      blockers.push(trend?.status === 'SETUP' ? 'Trend setup تشکیل شده ولی Trigger هنوز فعال نشده' : 'هیچ Trigger معتبر فعال نیست');
    }
  }
  ...
  const reg = marketRegime(d);
  const evaluatedQuality = signal ? evaluateIndependentQuality(signal, d, { active: activeStrategies }) : null;
  const quality = evaluatedQuality ? {
    ...evaluatedQuality,
    regime: reg,
    reasons: [...new Set([...(evaluatedQuality.reasons || []), ...(signal?.reason ? [signal.reason] : [])])],
    penalties: [...new Set([...(evaluatedQuality.blockers || []), ...blockers])]
  } : {
    score: 0, grade:'POOR', tradable:false, regime:reg, reasons:[], penalties:blockers, blockers:blockers
  };
  ...
  return {
    ...
    diagnostics: {
      ...
      minTriggerConfirmations: MIN_TRIGGER_CONFIRMATIONS,
      triggerConfirmation,
      qualityMode: 'EXECUTION_SAFETY_ONLY',
      trendSellFilter: 'H1 aligned + RSI<=48',

...
// Historical provider diagnostics are kept in memory for the backtest route.
...
function sanitizeProviderError(error) {
  ...
  return String(error).replace(/([?&]apikey=)[^&\s]+/gi, '$1[REDACTED]');
}

async function fetchTFHistory(tf, totalBars) {
  const key = process.env.TWELVEDATA_API_KEY;
  if (!key || !TF[tf]) {
    historyDiagnostics.set(tf, {
      requested: Number(totalBars) || 0,
      returned: 0,
      pages: 0,
      complete: false,
      providerLimited: false,
      stoppedReason: !key ? 'missing-api-key' : 'invalid-timeframe',
      provider: null,
      oldest: null,
      newest: null,
      pageDiagnostics: []
  ...
  const configuredChunk = Number(process.env.HISTORY_PAGE_SIZE || 1000);
  const chunk = Math.max(50, Math.min(5000, configuredChunk, wanted));
  ...
  for (let page = 0; page < 80 && all.size < wanted; page++) {
    ...
    const cacheKey = `engine-history-v6:${SYMBOL}:${tf}:${chunk}:${boundary}`;
    const result = await fetchJson(cacheKey, url, ttl, 30000);
    const data = result.data;

    lastProvider = {
      request: { symbol: SYMBOL, timeframe: tf, interval: TF[tf], outputsize: chunk, boundary },
      httpStatus: result.httpStatus ?? result.status ?? null,
      providerError: result.providerError ?? null,
      error: sanitizeProviderError(result.error ?? null),
      cached: !!result.cached,
      stale: !!result.stale
    };

    if (!data || data.status === 'error' || !Array.isArray(data.values) || !data.values.length) {
      if (result.providerError || data?.status === 'error') stoppedReason = 'provider-error';
      else if ((result.httpStatus ?? result.status) >= 400) stoppedReason = 'provider-http-error';
      else if (result.error) stoppedReason = 'network-error';
      else stoppedReason = 'provider-no-data';
    ...
    if (!bars.length) {
      ...
      pages.push({ page: page + 1, received: 0, uniqueAdded: 0, oldest: null, newest: null, error: { message: 'Provider returned values but none could be normalized.' } });
      break;
    }
    ...
    if (seenBoundaries.has(pageKey)) {
      ...
      pages.push({ page: page + 1, received: bars.length, uniqueAdded: 0, oldest: new Date(oldest).toISOString(), newest: new Date(newest).toISOString(), error: { message: 'Repeated provider page boundary.' } });
      break;
    }
    ...
    const nextEndMs = oldest - intervalMs;
  ...
  const meta = {
    ...
    providerLimited: out.length < wanted && ['provider-no-data', 'provider-http-error', 'provider-error'].includes(stoppedReason),
    stoppedReason,
...
function getHistoryMeta(tf) {
  return historyDiagnostics.get(tf) || null;
}
...
function decide(d, fund, news, options = {}) {
  ...
  const active = !!signal && signal.status === 'ACTIVE' && confidence >= MIN_CONFIDENCE && blockers.length === 0 && quality.tradable;
  return {
...
module.exports = {
  ...
  analyze, decide, fetchTF, fetchTFHistory, getHistoryMeta, fetchFundamental, fetchNewsRisk, neutralFundamental, neutralNewsRisk
};

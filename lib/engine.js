...
// Historical fetch for large backtests. TwelveData limits a single response,
// so older candles are fetched in chunks using end_date and then merged.
async function fetchTFHistory(tf, totalBars) {
  const key = process.env.TWELVEDATA_API_KEY;
  if (!key) return null;
  const wanted = Math.max(220, Math.min(Number(totalBars) || 220, 50000));
  if (wanted <= 4500) return fetchTF(tf, wanted);

  const chunk = 4500;
  const ttlByTf = { '15M': 3600000, '1H': 3600000, '4H': 3600000, 'Daily': 3600000 };
  const all = [];
  let endDate = null;

  while (all.length < wanted) {
    let url = `https://api.twelvedata.com/time_series?symbol=${encodeURIComponent(SYMBOL)}&interval=${TF[tf]}&outputsize=${chunk}&apikey=${key}`;
    if (endDate) url += `&end_date=${encodeURIComponent(endDate)}`;
    const cacheKey = `engine-history:${SYMBOL}:${tf}:${chunk}:${endDate || 'latest'}`;
    const result = await fetchJson(cacheKey, url, ttlByTf[tf]);
    const d = result.data;
    if (!d || d.status === 'error' || !Array.isArray(d.values) || !d.values.length) break;

    const bars = normalizeBars(d.values).sort((a, b) => a.time - b.time);
    if (!bars.length) break;
    const previousOldest = all.length ? all[0].time : Infinity;
    const unique = bars.filter(b => b.time < previousOldest);
    all.unshift(...unique);
    if (all.length >= wanted || unique.length === 0) break;

    const oldest = bars[0].time - 1;
    endDate = new Date(oldest).toISOString().replace('T', ' ').replace('.000Z', '');
    if (bars.length < chunk) break;
  }

  return all.slice(-wanted).sort((a, b) => a.time - b.time);
}

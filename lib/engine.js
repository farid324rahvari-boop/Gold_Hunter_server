...
//
// هدف:
// بک‌تست دقیقاً همان منطقی را اجرا کند که سیگنال زنده اجرا می‌کند.
// معماری فعلی: Independent Strategies
//
// نکته:
// Trend Following اولین استراتژی مستقل فعال است.
// سایر استراتژی‌ها فعلاً Diagnostic/Context هستند و رأی آن‌ها
// برای فعال‌کردن معامله جمع نمی‌شود.
...
// حداقل فاصله مجاز حد ضرر به دلار
const MIN_STOP_USD =
...
async function fetchTFHistory(
  ...
  const chunk =
    Math.min(
      5000,
      Number(
        process.env.HISTORY_PAGE_SIZE || 4500
      )
    );
  ...
  for (
    ...
    const cacheKey =
      `engine-history-v5:${SYMBOL}:${tf}:${chunk}:${boundary}`;
...
/* =========================================================
   SESSION
   FIX:
   backtest.js expects engine.getSession()
========================================================= */
...
function getSession(time) {
  ...
  /*
   * TwelveData historical timestamps are treated as UTC.
   *
   * 00:00–07:59 UTC  -> ASIA
   * 08:00–12:59 UTC  -> LONDON
   * 13:00–16:59 UTC  -> NEW_YORK
   * 17:00–23:59 UTC  -> NEW_YORK_LATE
   */
...
function trendStrategy(d) {
  ...
  if (
    ...
    return {
      ...base,
      reason:
        'Daily و 4H روند هم‌جهت و معتبر ندارند'
    };
  }
  ...
  if (
    ...
    return {
      ...
      direction: dir,
      reason:
        'برای تشخیص Pullback/Continuation داده 15M کافی نیست'
    };
  }
  ...
  if (!setup) {
    return {
      ...
      reason:
        `روند ${dir === 'BUY' ? 'صعودی' : 'نزولی'} است اما Pullback/شرایط ورود کامل نشده`,

      trigger:
        dir === 'BUY'
          ? '15M کلوز بالای سقف Trigger پس از Pullback'
          : '15M کلوز زیر کف Trigger پس از Pullback'
    };
  }
  ...
  if (!triggered) {
    return {
      ...
      reason:
        `روند ${dir === 'BUY' ? 'صعودی' : 'نزولی'} + Pullback معتبر؛ منتظر Continuation 15M`,
  ...
  if (!hasSpace) {
    return {
      ...
      trigger:
        'پس از ایجاد فضای کافی دوباره بررسی شود'
    };
  }
  ...
  return {
    ...
    reason:
      `Trend ${dir === 'BUY' ? 'BUY' : 'SELL'}: Daily+4H هم‌جهت، Pullback و Continuation تأیید شد`,
...
function runStrategies(
  ...
  /*
   * IMPORTANT:
   * این استراتژی‌ها دیگر با هم جمع نمی‌شوند.
   * فعلاً فقط Trend Following می‌تواند Trigger ایجاد کند.
   */
...
function scoreSignalQuality(
  ...
  if (
    ...
    score += 3;
    reasons.push(
      'یکی از تایم‌فریم‌های بالا هم‌جهت است'
    );

  } else {
    score -= 6;
    penalties.push(
      'تایم‌فریم‌های بالا خلاف جهت‌اند'
    );
  }

  /*
   * Compatibility only.
   * Consensus دیگر شرط ورود نیست.
   */
...
function buildTrade(
  ...
  if (
    ...
    trigger =
      '15M: Sweep Low ترجیحاً + BOS/CHoCH صعودی + کلوز بالای EMA20 + RSI>50';
    ...
    trigger =
      '15M: Sweep High ترجیحاً + BOS/CHoCH نزولی + کلوز زیر EMA20 + RSI<50';
  }
...
function decide(
  ...
  /*
   * IMPORTANT:
   * هیچ Weighted Voting وجود ندارد.
   *
   * یک استراتژی مستقل اگر قوانین خودش را
   * کامل کند می‌تواند Trigger شود.
   *
   * در نسخه فعلی Trend Following اولین
   * استراتژی‌ای است که اجازه Trigger دارد.
   */
   ...
   * این فیلدها برای UI/backtest باقی مانده‌اند.
   * اما دیگر شرط ورود نیستند.
   */
...
module.exports = {
  ...
  // FIX:
  // Required by historical backtest route.
  getSession
};

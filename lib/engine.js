...
function fibZone(d) {
  ...
  const pos = (price - swingLow) / span;
  const bullishTrend = price > d.m15.ema20 && d.m15.rsi > 50;
  const bearishTrend = price < d.m15.ema20 && d.m15.rsi < 50;
  // Fibonacci is context, not a standalone directional vote.
  // Retracement near 38.2–61.8% earns a small bonus only when momentum agrees.
  const inZone = pos >= 0.382 && pos <= 0.618;
  if (inZone && bullishTrend) return { vote: 'BUY', score: 1, reason: `قیمت در ناحیه اصلاحی 38.2–61.8% با مومنتوم صعودی` };
  if (inZone && bearishTrend) return { vote: 'SELL', score: -1, reason: `قیمت در ناحیه اصلاحی 38.2–61.8% با مومنتوم نزولی` };
  return { vote: 'NEUTRAL', score: 0, reason: 'فیبوناچی تأیید جهت مستقلی نمی‌دهد' };
}
...
// Historical provider throttle: keep requests spaced so a backtest cannot
// accidentally burn the whole provider quota in a burst.
let lastHistoryProviderRequestAt = 0;
let historyThrottlePromise = Promise.resolve();
const HISTORY_MIN_GAP_MS = Math.max(1000, Number(process.env.HISTORY_MIN_GAP_MS || 10000));

function waitHistoryThrottle() {
  historyThrottlePromise = historyThrottlePromise.then(async () => {
    const wait = Math.max(0, HISTORY_MIN_GAP_MS - (Date.now() - lastHistoryProviderRequestAt));
    if (wait > 0) await new Promise(r => setTimeout(r, wait));
    lastHistoryProviderRequestAt = Date.now();
  });
...
async function fetchTFHistory(tf, totalBars) {
  ...
  // Large pages reduce the number of provider calls for long backtests.
  // A small request such as data-test?bars=220 still makes only one 220-bar call.
  const configuredChunk = Number(process.env.HISTORY_PAGE_SIZE || 5000);
  const chunk = Math.max(220, Math.min(5000, configuredChunk));
  const ttl = Math.max(6 * 60 * 60 * 1000, Number(process.env.HISTORY_CACHE_TTL_MS || 24 * 60 * 60 * 1000));
  const all = new Map();
  const pages = [];
  ...
  for (let page = 0; page < 80 && all.size < wanted; page++) {
    const pageSize = Math.min(chunk, wanted - all.size);
    let url =
    ...
    if (endMs !== null) url += `&end_date=${encodeURIComponent(formatProviderDate(endMs))}`;

    const boundary = endMs === null ? 'latest' : formatProviderDate(endMs);
    const cacheKey = `engine-history-v6:${SYMBOL}:${tf}:${pageSize}:${boundary}`;

    // Historical pages are immutable for our purposes. Cache them for 24h by default.
    // fetchJson also deduplicates simultaneous requests for the exact same page.
    let result = await fetchJson(cacheKey, url, ttl);
    let data = result.data;

    // If there is no cached result, throttle the provider request. This is deliberately
    // after cache lookup so normal repeated calls cost zero provider requests.
    if (!result.cached && !data) {
      await waitHistoryThrottle();
      result = await fetchJson(cacheKey, url, ttl);
      data = result.data;
    }

    if (!data || data.status === 'error' || !Array.isArray(data.values) || !data.values.length) {
      stoppedReason = result.status === 429 || result.providerError ? 'provider-error' : 'provider-no-more-data';
      pages.push({
        page: page + 1,
        received: 0,
        uniqueAdded: 0,
        ...
        stale: !!result.stale,
        error: {
          request: { symbol: SYMBOL, timeframe: tf, interval: TF[tf], outputsize: pageSize, boundary },
          httpStatus: result.status || null,
          providerError: result.providerError || data?.message || null,
    ...
    if (!bars.length) { stoppedReason = 'invalid-page'; break; }
  ...
  const out = Array.from(all.values()).sort((a,b) => a.time - b.time).slice(-wanted);
  const meta = {
    requested: wanted,
    returned: out.length,
    pages: pages.length,
    ...
    stoppedReason,
    cachePolicy: { ttlMs: ttl, pageSize: chunk, minGapMs: HISTORY_MIN_GAP_MS },
  ...
  Object.defineProperty(out, 'historyMeta', { value: meta, enumerable: false, configurable: false });
...
function trendStrategy(d) {
  ...
  if (dir === 'BUY') {
    ...
    if (price - sl < MIN_STOP_USD) sl = price - MIN_STOP_USD;
    risk = price - sl;
  } else {
    sl = pullbackHigh + atrValue * 0.15;
    if (sl - price < MIN_STOP_USD) sl = price + MIN_STOP_USD;
    risk = sl - price;
  }
...
function runStrategies(d, fund) {
  ...
  const structureVote = d.m15.structure === 'BULLISH_BOS' ? 'BUY' : d.m15.structure === 'BEARISH_BOS' ? 'SELL' : 'NEUTRAL';
  const sweepVote = d.m15.sweep === 'SWEEP_LOW' ? 'BUY' : d.m15.sweep === 'SWEEP_HIGH' ? 'SELL' : 'NEUTRAL';
  ...
  // IMPORTANT: these are diagnostics only. They are deliberately not summed.
  const strategies = [
    trend,
    { name:'ساختار بازار (BOS/CHoCH)', strategyId:'STRUCTURE', status:structureVote==='NEUTRAL'?'WATCH':'SETUP', vote:structureVote, direction:structureVote, confidence:0, weight:0, reason:structureVote==='BUY'?'BOS صعودی 15M':structureVote==='SELL'?'BOS نزولی 15M':'ساختار 15M آمیخته' },
    { name:'Liquidity Sweep (SMC)', strategyId:'LIQUIDITY_SWEEP', status:sweepVote==='NEUTRAL'?'WATCH':'SETUP', vote:sweepVote, direction:sweepVote, confidence:0, weight:0, reason:sweepVote==='BUY'?'Sweep نقدینگی زیر کف و بازگشت':sweepVote==='SELL'?'Sweep نقدینگی بالای سقف و بازگشت':'Sweep فعال وجود ندارد' },
    { name:'مومنتوم (RSI + EMA20)', strategyId:'MOMENTUM', status:momVote==='NEUTRAL'?'WATCH':'SETUP', vote:momVote, direction:momVote, confidence:0, weight:0, reason:momVote==='BUY'?`RSI ${fmt(d.m15.rsi)} بالای 50 و قیمت بالای EMA20`:momVote==='SELL'?`RSI ${fmt(d.m15.rsi)} زیر 50 و قیمت زیر EMA20`:'مومنتوم خنثی/افراطی' },
    { name:'Fibonacci Retracement', strategyId:'FIBONACCI', status:'WATCH', vote:'NEUTRAL', direction:'WAIT', confidence:0, weight:0, contextVote:fib.vote, reason:fib.reason },
    { name:'واگرایی RSI', strategyId:'RSI_DIVERGENCE', status:'WATCH', vote:'NEUTRAL', direction:'WAIT', confidence:0, weight:0, contextVote:div.vote, reason:div.reason },
    { name:'فاندامنتال (FRED)', strategyId:'FUNDAMENTAL', status:fundVote==='NEUTRAL'?'WATCH':'SETUP', vote:fundVote, direction:fundVote, confidence:0, weight:0, reason:fund.items?.length ? fund.items.map(i=>i.why).join('، ') : 'فاندامنتال تاریخی/کافی در این نقطه موجود نیست' }
  ];

  return {
    strategies,
    volumeConfirmed: !!d.m15.volumeState.confirmed,
    triggerStrategy: trend.status === 'ACTIVE' ? trend : null,
    context: { fib, divergence:div, sweep:{vote:sweepVote}, structure:{vote:structureVote}, h4Trend:trendBias(d.h4), d1Trend:trendBias(d.daily), h1Trend:trendBias(d.h1) }
  };
}
...
function buildTrade(d, dir) {
  ...
  if(dir==='BUY'){
    ...
    if(entryHigh-sl<MIN_STOP_USD) sl=entryHigh-MIN_STOP_USD;
    ...
    if(sl-entryLow<MIN_STOP_USD) sl=entryLow+MIN_STOP_USD;
...
function decide(d, fund, news, options = {}) {
  ...
  // New architecture: there is NO vote/weight aggregation. A strategy can
  // independently activate a trade when its own complete rule-set is valid.
  // Until the other engines are rewritten one-by-one, Trend Following is the
  // first independent engine allowed to trigger a position.
  ...
  return {
    ...
    diagnostics: {
      ...
      firstEngineImplemented: 'TREND_FOLLOWING',
      otherEnginesAreDiagnosticOnly: true
    }
  };
}
...
module.exports = {
  ...
  analyze, decide, fetchTF, fetchTFHistory, getHistoryMeta, getSession, fetchFundamental, fetchNewsRisk, neutralFundamental, neutralNewsRisk
};

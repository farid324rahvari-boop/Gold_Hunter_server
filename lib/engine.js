...
function scoreSignalQuality(
  ...
  /*
   * Quality دیگر تمام شرایط تکنیکال را Block نمی‌کند.
   * فقط موارد واقعاً خطرناک Blocker هستند.
   */
  const tradable =
...
function decide(
  ...
  /*
   * Blockerهای اصلی موتور
   * فقط شرایط واقعاً نامناسب
   */
  if (
  ...
  /*
   * کیفیت دیگر blockerهای خودش را
   * به blockerهای اصلی اضافه نمی‌کند.
   *
   * بنابراین Quality نقش فیلتر نرم دارد.
   */
  const active =
    dir !== 'WAIT' &&

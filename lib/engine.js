...
function structureStrategy(d) {
  ...
  if (v === 'WAIT') return { name:'ساختار بازار (BOS/CHoCH)', strategyId:'STRUCTURE', status:'WATCH', vote:'NEUTRAL', direction:'WAIT', confidence:0, weight:0, reason:'ساختار 15M آمیخته' };
  const mom = v === 'BUY' ? d.m15.rsi >= 50 && d.m15.price > d.m15.ema20 : d.m15.rsi <= 50 && d.m15.price < d.m15.ema20;
  const h4 = trendBias(d.h4), d1 = trendBias(d.daily);
  let c = 62;
  if (mom) c += 12;
  if (h4 === (v === 'BUY' ? 1 : -1)) c += 8;
  if (d1 === (v === 'BUY' ? 1 : -1)) c += 8;
  const trig = v === 'BUY' ? 'BOS صعودی 15M + کلوز بالای EMA20' : 'BOS نزولی 15M + کلوز زیر EMA20';
  if (!mom) return { name:'ساختار بازار (BOS/CHoCH)', strategyId:'STRUCTURE', status:'SETUP', vote:v, direction:v, confidence:Math.round(clamp(c)), weight:0, reason:'BOS شناسایی شد اما مومنتوم هنوز تأیید کامل نیست', trigger:trig };
  return independentTradeStrategy({name:'ساختار بازار (BOS/CHoCH)',strategyId:'STRUCTURE'}, d, v, c, v==='BUY'?'BOS صعودی + مومنتوم صعودی':'BOS نزولی + مومنتوم نزولی', trig);
}
...
function sweepStrategy(d) {
  ...
  const mom = v === 'BUY' ? d.m15.price > d.m15.ema20 && d.m15.rsi > 50 : d.m15.price < d.m15.ema20 && d.m15.rsi < 50;
  let c = 68 + (mom ? 15 : 0);
...
function fibonacciStrategy(d, fib) {
  if (!fib || !['BUY','SELL'].includes(fib.vote)) return { name:'Fibonacci Retracement', strategyId:'FIBONACCI', status:'WATCH', vote:'NEUTRAL', direction:'WAIT', confidence:0, weight:0, contextVote:fib?.vote||'NEUTRAL', reason:fib?.reason||'فیبوناچی خنثی' };
  const dir=fib.vote;
  const aligned=dir==='BUY' ? d.m15.price>d.m15.ema20 && d.m15.rsi>50 : d.m15.price<d.m15.ema20 && d.m15.rsi<50;
  let c=66+(aligned?14:0);
  const trig=dir==='BUY'?'ورود به 38.2–61.8% + EMA20/RSI تأیید':'ورود به 38.2–61.8% + EMA20/RSI تأیید';
  if(!aligned) return { name:'Fibonacci Retracement',strategyId:'FIBONACCI',status:'SETUP',vote:dir,direction:dir,confidence:c,weight:0,contextVote:dir,reason:'ناحیه فیبوناچی دیده شد ولی مومنتوم هم‌جهت نیست',trigger:trig };
...
function divergenceStrategy(d, div) {
  if (!div || !['BUY','SELL'].includes(div.vote)) return { name:'واگرایی RSI',strategyId:'RSI_DIVERGENCE',status:'WATCH',vote:'NEUTRAL',direction:'WAIT',confidence:0,weight:0,contextVote:div?.vote||'NEUTRAL',reason:div?.reason||'واگرایی مشخص نیست' };
  const dir=div.vote;

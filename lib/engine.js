...
// Improved signal quality + calibrated confidence + consistent trade model
...
function computeConsensus(
  ...
  /*
   * قبلاً confidence تا 100 خیلی راحت می‌رفت.
   * حالا دامنه منطقی‌تر شده:
   *
   * 50 = بدون برتری
   * 72 = قابل بررسی
   * 80 = قوی
   * 90 = بسیار قوی
   *
   * رسیدن به 100 تقریباً غیرممکن است.
   */
  let confidence =
    50 +
    (
...
function buildTrade(
  ...
  /*
   * Entry اصلی = قیمت واقعی فعلی.
   * این بخش با بک‌تست هماهنگ شده.
   */
  ...
  if (dir === 'BUY') {
    ...
    /*
     * اگر مقاومت نزدیک باشد،
     * تارگت را پشت مقاومت نمی‌بریم.
     */
    if (
...
function scoreSignalQuality(
  ...
  /*
   * 10. Location نسبت به S/R
   *
   * اگر BUY دقیقاً زیر مقاومت باشد
   * یا SELL دقیقاً بالای حمایت باشد،

...
// نسخه کالیبره Trend Following v2
//
// تغییرات این نسخه:
// 1) استراتژی‌ها مستقل هستند و اجماع شرط ورود نیست.
// 2) Trend Following تنها Engine فعال برای ورود است.
// 3) حداقل R:R واقعی برای ACTIVE قابل تنظیم است.
// 4) سشن Asia فیلتر سخت‌تری دارد ولی حذف نشده.
// 5) Confidence دوباره کالیبره شده تا 98-100 بیش از حد تولید نشود.
// 6) سایر استراتژی‌ها همچنان Diagnostic Only هستند.
// 7) Live و Backtest از یک منطق استفاده می‌کنند.
...
// ============================================================
// ACTIVE TRADE CALIBRATION
// ============================================================

// حداقل R:R واقعی برای فعال شدن معامله.
// قبلاً MIN_RR=1.8 برای ACTIVE کافی بود.
// اکنون به طور پیش‌فرض 2.5 شده است.
const TREND_MIN_ACTIVE_RR =
  Number(
    process.env.TREND_MIN_ACTIVE_RR || 2.5
  );

// حداقل R:R مخصوص Asia.
// آسیا حذف نشده، اما چون بک‌تست نشان داد کیفیت پایین‌تری دارد,
// فقط setupهایی با فضای بهتر اجازه ACTIVE می‌گیرند.
const TREND_ASIA_MIN_RR =
  Number(
    process.env.TREND_ASIA_MIN_RR || 3.0
  );

// اگر true باشد، فیلتر Asia فعال است.
const TREND_ASIA_FILTER_ENABLED =
  String(
    process.env.TREND_ASIA_FILTER_ENABLED || 'true'
  ) !== 'false';
...
// ============================================================
// TREND FOLLOWING — CALIBRATION PARAMETERS
// ============================================================
...
const TREND_PULLBACK_LOOKBACK =
  Math.max(
    6,
    Number(
      process.env.TREND_PULLBACK_LOOKBACK || 12
    )
  );

const TREND_PULLBACK_ATR =
  Number(
    process.env.TREND_PULLBACK_ATR || 0.55
  );

const TREND_MAX_CHASE_ATR =
  Number(
    process.env.TREND_MAX_CHASE_ATR || 1.45
  );

const TREND_BUY_RSI_MIN =
  Number(
    process.env.TREND_BUY_RSI_MIN || 46
  );

const TREND_BUY_RSI_MAX =
  Number(
    process.env.TREND_BUY_RSI_MAX || 78
  );

const TREND_SELL_RSI_MIN =
  Number(
    process.env.TREND_SELL_RSI_MIN || 22
  );

const TREND_SELL_RSI_MAX =
  Number(
    process.env.TREND_SELL_RSI_MAX || 54
  );

const TREND_MIN_SETUP_SCORE =
  Number(
    process.env.TREND_MIN_SETUP_SCORE || 60
  );

const TREND_ALLOW_SINGLE_HTF_WITH_H1 =
  String(
    process.env.TREND_ALLOW_SINGLE_HTF_WITH_H1 || 'true'
  ) !== 'false';

const TREND_REQUIRE_HTF_ALIGNMENT =
  String(
    process.env.TREND_REQUIRE_HTF_ALIGNMENT || 'false'
  ) === 'true';

const TREND_MIN_ATR_USD =
  Number(
    process.env.TREND_MIN_ATR_USD || 0
  );
...
// 50 + score * 0.45
//
// باعث می‌شد حتی setupهای متوسط هم خیلی سریع به 90-100 برسند.
//
// مدل جدید:
// پایه پایین‌تر است و bonusها محدودتر شده‌اند.
// هدف: confidence بیشتر نقش کیفیت setup را داشته باشد
// و با win-rate اشتباه گرفته نشود.

const CONFIDENCE_BASE =
  Number(
    process.env.TREND_CONFIDENCE_BASE || 35
  );

const CONFIDENCE_SCORE_FACTOR =
  Number(
    process.env.TREND_CONFIDENCE_SCORE_FACTOR || 0.40
  );

const CONFIDENCE_FULL_HTF_BONUS =
  Number(
    process.env.TREND_CONFIDENCE_FULL_HTF_BONUS || 5
  );

const CONFIDENCE_SINGLE_HTF_BONUS =
  Number(
    process.env.TREND_CONFIDENCE_SINGLE_HTF_BONUS || 2
  );

const CONFIDENCE_PULLBACK_BONUS =
  Number(
    process.env.TREND_CONFIDENCE_PULLBACK_BONUS || 3
  );

const CONFIDENCE_CONTINUATION_BONUS =
  Number(
    process.env.TREND_CONFIDENCE_CONTINUATION_BONUS || 3
  );

const CONFIDENCE_NOT_CHASING_BONUS =
  Number(
    process.env.TREND_CONFIDENCE_NOT_CHASING_BONUS || 2
  );

const CONFIDENCE_MAX =
  Number(
    process.env.TREND_CONFIDENCE_MAX || 92
  );
...
let lastHistoryProviderRequestAt =
  0;

let historyThrottlePromise =
  Promise.resolve();
...
const HISTORY_MIN_GAP_MS =
  Math.max(
    1000,
    Number(
      process.env.HISTORY_MIN_GAP_MS ||
      10000
    )
  );
...
function trendStrategy(d) {
  ...
  if (
    ...
    return {
      ...
      reason:
        `R:R محاسبه‌شده کمتر از حد پایه ${MIN_RR} است`,
  ...
  // حداقل R:R لازم برای ACTIVE شدن.
  // این قسمت از MIN_RR عمومی جدا شده است.
  //
  // MIN_RR:
  // حداقل سازگاری عمومی سیستم.
  //
  // TREND_MIN_ACTIVE_RR:
  // حداقل R:R برای اینکه Trend واقعاً معامله پیشنهاد کند.
  ...
  if (
    ...
    return {
      ...
      reason:
        session === 'ASIA'
          ? `Trend ${dir} فعال است ولی R:R ${fmt(rr)} برای سشن Asia کافی نیست؛ حداقل لازم ${fmt(activeMinRR)} است`
          : `Trend ${dir} فعال است ولی R:R ${fmt(rr)} برای ACTIVE شدن کافی نیست؛ حداقل لازم ${fmt(activeMinRR)} است`,

      trigger:
        'منتظر ایجاد فضای بیشتر برای هدف / R:R بهتر',
  ...
  return {
    ...
    blockers: [
      ...new Set(
        penalties.filter(
          x =>
            /خبر|R:R|ضرر|خلاف جهت|فاصله ورود/.test(
              x
            )
        )
      )
...
function decide(
  ...
  if (
    ...
    return {
      ...
      confidence: 0,
      price: null,
      reason: 'داده کافی برای تصمیم‌گیری وجود ندارد',
      trigger: null,
      trade: null,
      blockers: ['داده تایم‌فریم‌های لازم موجود نیست'],
      strategies: [],
      activeStrategies: [],
      consensus: {
      ...
      marketRegime: null,
      fundamental: fund || neutralFundamental(),
      news: news || neutralNewsRisk(),
      volumeConfirmed: false,
      context: {}
    };
  }

  fund = fund || neutralFundamental();
  news = news || neutralNewsRisk();

  const result = runStrategies(d, fund);
  const strategies = result.strategies || [];
  const triggerStrategy = result.triggerStrategy || null;
  const activeStrategies = strategies.filter(s => s.status === 'ACTIVE');
  const volumeConfirmed = !!result.volumeConfirmed;
  const context = result.context || {};

  const trend = strategies.find(
    s => s.strategyId === 'TREND_FOLLOWING'
  ) || null;

  const dir = triggerStrategy?.direction || trend?.direction || 'WAIT';

  const trade = triggerStrategy
    ? {
        entry: triggerStrategy.entry || null,
        stopLoss: triggerStrategy.stopLoss ?? null,
        targets: triggerStrategy.targets || [],
        rr: Number(triggerStrategy.rr || 0),
        trigger: triggerStrategy.trigger || null,
        invalidation: triggerStrategy.invalidation || null,
        levels: triggerStrategy.levels || null
      }
    : null;
  ...
  if (
    triggerStrategy &&
    Number(triggerStrategy.confidence || 0) < Number(minConfidence)
  ) {
    blockers.push(
      `اعتماد استراتژی (${fmt(triggerStrategy.confidence)}) کمتر از حداقل ${fmt(minConfidence)} است`
    );
  }

  if (news?.blocked) {
    blockers.push('خبر پرریسک در پنجره ورود');
  }

  const consensus = computeConsensus(strategies, volumeConfirmed);

  const quality = triggerStrategy
    ? scoreSignalQuality(
        d,
        { ...consensus, dir },
        context,
    ...
    : {
        ...
        grade: 'POOR',
        tradable: false,
        regime: marketRegime(d),
        reasons: [],
        penalties: [],
        blockers: []
      };

  if (triggerStrategy && news?.blocked) {
    return {
      status: 'WAIT',
      decision: 'WAIT',
      signal: 'WAIT',
      direction: dir,
      confidence: Math.min(Number(triggerStrategy.confidence || 0), 69),
      price: d.m15.price,
      reason: 'استراتژی Trend فعال شده اما به دلیل خبر پرریسک ورود موقتاً مسدود است',
      trigger: triggerStrategy.trigger || null,
      trade,
      ...
      consensus,
      quality,
      marketRegime: marketRegime(d),
      fundamental: fund,
      news,
      volumeConfirmed,
      context,
      diagnostics: triggerStrategy.diagnostics || null
    };
  }
  ...
  if (
    ...
    return {
      ...
      signal: 'WAIT',
      direction: dir,
      confidence: Number(triggerStrategy.confidence || 0),
      price: d.m15.price,
      reason: 'استراتژی Trend setup معتبر دارد اما Confidence هنوز به حداقل لازم نرسیده است',
      trigger: triggerStrategy.trigger || null,
      trade,
      blockers: [...new Set(blockers)],
      strategies,
  ...
  if (!triggerStrategy) {
    return {
      ...
      confidence: Math.min(Number(trend?.confidence || 0), 71),
      price: d.m15.price,
      reason: trend?.reason || 'هیچ Trigger فعال مستقلی وجود ندارد',
      trigger: trend?.trigger || null,
      trade: null,
      blockers: [...new Set(blockers)],
      strategies,
      activeStrategies,
      ...
      volumeConfirmed,
      context,
      diagnostics: trend?.diagnostics || null
    };
  }
  ...
  return {
    ...
    confidence: Number(triggerStrategy.confidence || 0),
    price: d.m15.price,
    reason: triggerStrategy.reason,
    trigger: triggerStrategy.trigger,
    trade: {
      ...trade,
      isHypothetical: false
    },
    entry: triggerStrategy.entry,
    stopLoss: triggerStrategy.stopLoss,
    targets: triggerStrategy.targets,
    rr: triggerStrategy.rr,
    invalidation: triggerStrategy.invalidation,
    levels: triggerStrategy.levels,
    blockers: [...new Set(blockers)],
    strategies,
    activeStrategies,
    consensus,

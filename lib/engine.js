...
function structure(b) {
  // A valid BOS must be a close-based break of the previous structure,
  // with meaningful displacement. Merely making a wick above/below a range
  // is not enough; that was producing too many weak Structure triggers.
  if (b.length < 26) return 'MIXED';
  const x = b.slice(-6), p = b.slice(-20, -6);
  if (x.length < 6 || p.length < 12) return 'MIXED';
  ...
  const rangeSize = Math.max(last.high - last.low, 1e-9);
  const closeLocation = (last.close - last.low) / rangeSize;
  const bullBreak = last.close > ph;
  const bearBreak = last.close < pl;
  const bullDisplacement = body >= a * 0.35 && closeLocation >= 0.65;
  const bearDisplacement = body >= a * 0.35 && closeLocation <= 0.35;
  if (bullBreak && bullDisplacement) return 'BULLISH_BOS';
  if (bearBreak && bearDisplacement) return 'BEARISH_BOS';
  return 'MIXED';
}
